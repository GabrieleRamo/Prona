...
function seedListings(){
  ...
  return [
    ...
    if(!l.photos.length){
      ...
      const pool=Array.from({length:12},(_,i)=>`property-photos/p${String(i+1).padStart(2,"0")}.jpg`);
      let s=(seedNum*2654435761)%2147483647||7; const rnd=()=>((s=s*48271%2147483647)/2147483647);
      const shuffled=pool.map(p=>[rnd(),p]).sort((a,b)=>a[0]-b[0]).map(x=>x[1]);
      const n=3+(seedNum%2);
      l.photos=shuffled.slice(0,n);
    }
...
function viewPartners(){
  ...
  const draw=q=>{
    ...
    grid.innerHTML=list.map(p=>{
      ...
      const tier=p.listings.some(l=>l.ownerPlan==="premium")?"premium":p.listings.some(l=>l.ownerPlan==="pro")?"pro":"verified";
      return `<div class="partner-card">
        ${avatarHTML(p.name,p.avatarPhoto,"partner-avatar")}
        <b class="partner-name">${esc(p.name)}</b>
        <span class="tag ${tier}" style="margin-bottom:6px">${tier==="premium"?"PREMIUM":tier==="pro"?"PRO":"VERIFIED"}</span>
...
async function viewPartnerProfile(key){
  ...
  const tier=p.listings.some(l=>l.ownerPlan==="premium")?"premium":p.listings.some(l=>l.ownerPlan==="pro")?"pro":"verified";
  const tierLabel=tier==="premium"?"PREMIUM":tier==="pro"?"PRO":"VERIFIED";
  v.innerHTML=`<div class="static-page partner-profile-page"><div class="inner" style="max-width:960px">
    ...
    <div class="pp-header">
      ...
      <div class="pp-head-info">
        <h1>${esc(p.name)} <span class="tag ${tier}">${tierLabel}</span></h1>
