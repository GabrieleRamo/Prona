...
/* approximate EUR<->ALL (Lek) rate for normalizing price filters across mixed-currency listings — not a live rate */
const EUR_TO_ALL=100;
const priceInEUR=l=>l.currency==="ALL"?l.price/EUR_TO_ALL:l.price;
/* data.js (map geometry: buildings/roads/water/country outline) is large (~12MB) and only needed
   once a map actually renders — load it async in the background instead of blocking initial paint. */
let GEO_READY=null;
function loadGeoData(){
  if(!GEO_READY){
    GEO_READY=new Promise((resolve,reject)=>{
      if(window.GEO){resolve(window.GEO);return;}
      const s=document.createElement("script");
      s.src="data.js";
      s.onload=()=>resolve(window.GEO);
...
loadGeoData(); // kick off immediately in the background; map-creating views await this before touching GEO
...
function roomSVG(kind,seed){
  ...
  const palettes=[["#c62f2c","#f4e3df"],["#2d5fb8","#e3ecf7"],["#2e7d5b","#e2f0e9"],["#b8860b","#f6ecd9"]];
  const [accent,tint]=palettes[Math.floor(rnd()*palettes.length)];
  const wall="#f2efe8", floor="#e2dccf";
  let body="";
  if(kind==="exterior"){
    body=`<rect width="400" height="300" fill="#bcd4e6"/><rect y="210" width="400" height="90" fill="#d8d2c4"/>
      <rect x="60" y="60" width="280" height="150" rx="4" fill="#e7e1d4" stroke="#cfc8b8"/>
      ${Array.from({length:4},(_,r)=>Array.from({length:6},(_,c)=>`<rect x="${76+c*44}" y="${76+r*32}" width="26" height="20" fill="${rnd()>0.4?tint:accent}" opacity="${rnd()>0.4?1:.85}"/>`).join("")).join("")}
      <rect x="170" y="170" width="60" height="40" fill="#8a6a4a"/>`;
  } else if(kind==="kitchen"){
    body=`<rect width="400" height="300" fill="${wall}"/><rect y="230" width="400" height="70" fill="${floor}"/>
      <rect x="20" y="150" width="360" height="30" fill="#d8d2c4"/>
      ${Array.from({length:6},(_,i)=>`<rect x="${20+i*60}" y="180" width="56" height="70" fill="#fff" stroke="#d8d2c4"/>`).join("")}
      <rect x="20" y="60" width="360" height="70" fill="${tint}"/>
      ${Array.from({length:6},(_,i)=>`<rect x="${24+i*60}" y="64" width="52" height="62" fill="#fff" opacity=".6"/>`).join("")}
      <circle cx="120" cy="150" r="8" fill="${accent}"/><circle cx="150" cy="150" r="8" fill="${accent}"/>`;
  } else if(kind==="bedroom"){
    body=`<rect width="400" height="300" fill="${wall}"/><rect y="220" width="400" height="80" fill="${floor}"/>
      <rect x="100" y="120" width="220" height="100" rx="8" fill="#fff" stroke="#d8d2c4"/>
      <rect x="100" y="120" width="220" height="30" rx="8" fill="${accent}"/>
      <rect x="110" y="105" width="60" height="30" rx="6" fill="#fff" stroke="#d8d2c4"/><rect x="180" y="105" width="60" height="30" rx="6" fill="#fff" stroke="#d8d2c4"/>
      <rect x="60" y="150" width="30" height="40" fill="${tint}"/><circle cx="75" cy="140" r="10" fill="#f4e3b0"/>
      <rect x="300" y="40" width="70" height="70" fill="#bcd4e6" opacity=".7"/>`;
  } else if(kind==="bathroom"){
    body=`<rect width="400" height="300" fill="#eef3f2"/><rect y="230" width="400" height="70" fill="#dfe7e5"/>
      <rect x="40" y="150" width="140" height="70" rx="18" fill="#fff" stroke="#cfd8d6"/>
      <circle cx="300" cy="120" r="34" fill="#fff" stroke="#cfd8d6"/><rect x="270" y="150" width="60" height="16" fill="#fff" stroke="#cfd8d6"/>
      <rect x="285" y="60" width="30" height="50" fill="${tint}"/>
      <rect x="270" y="200" width="60" height="18" rx="4" fill="${accent}" opacity=".85"/>`;
  } else if(kind==="balcony"){
    body=`<rect width="400" height="300" fill="#bcd4e6"/><rect y="230" width="400" height="70" fill="#d8d2c4"/>
      <rect x="0" y="150" width="400" height="20" fill="#fff" opacity=".5"/>
      ${Array.from({length:14},(_,i)=>`<rect x="${10+i*28}" y="150" width="4" height="70" fill="#c9c2b0"/>`).join("")}
      <rect x="60" y="190" width="50" height="50" rx="6" fill="${accent}"/><circle cx="280" cy="210" r="22" fill="${tint}"/>`;
  } else {
    body=`<rect width="400" height="300" fill="${wall}"/><rect y="210" width="400" height="90" fill="${floor}"/>
      <rect x="30" y="40" width="120" height="90" fill="#bcd4e6" opacity=".8"/><rect x="30" y="40" width="120" height="90" fill="none" stroke="#fff" stroke-width="6"/>
      <rect x="120" y="180" width="200" height="70" rx="14" fill="${accent}"/><rect x="120" y="180" width="200" height="24" rx="12" fill="#fff" opacity=".18"/>
      <rect x="150" y="240" width="140" height="14" rx="4" fill="${tint}"/>
      <circle cx="340" cy="60" r="26" fill="#fff" opacity=".5"/>`;
  }
  return "data:image/svg+xml,"+encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300">${body}</svg>`);
}
function demoAvatar(name){ return `https://i.pravatar.cc/160?u=${encodeURIComponent(name)}`; }
function avatarHTML(name,photo,extraClass){
  const cls="avatar"+(extraClass?" "+extraClass:"");
  return photo?`<img class="${cls} avatar-photo" src="${photo}" alt="${esc(name)}">`:`<span class="${cls}">${esc(initials(name))}</span>`;
}
...
function seedListings(){
  const AGENTS=[
    {name:"Arben Hoxha — City Estates",type:"agent",phone:"+355 69 200 1001",whatsapp:true},
    {name:"Elira Krasniqi — Adriatik Immo",type:"agent",phone:"+355 69 200 1002",whatsapp:true},
    {name:"Redon Basha — Coastal Homes",type:"agency",phone:"+355 69 200 1003",whatsapp:false},
    {name:"Sara Gjika — Prona Ekskluzive",type:"agent",phone:"+355 69 200 1004",whatsapp:true},
  ];
  const mk=(i,city,dealType,propertyType,title,price,area,living,beds,rooms,floor,floors,dx,dy,feats,agent)=>{
    const ag=agent?AGENTS[i%AGENTS.length]:null;
    return {
    id:"seed-"+i, owner:null, seeded:true, status:"published",
    accountType:ag?ag.type:"owner", dealType, propertyType,
    ...
    contactName:ag?ag.name:"Pronar privat", phone:ag?ag.phone:"+355 69 000 00"+(i%10), whatsapp:ag?ag.whatsapp:i%2===0,
  ...
  return [
    ...
    if(!l.photos.length){
      const seedNum=parseInt(l.id.replace(/\D/g,""))||1;
      const kinds=["living","kitchen","bedroom","exterior","bathroom","balcony"];
      const n=3+(seedNum%2);
      const start=seedNum%kinds.length;
      l.photos=Array.from({length:n},(_,i)=>roomSVG(kinds[(start+i)%kinds.length],seedNum*7+i));
    }
    if(l.accountType==="agent"||l.accountType==="agency") l.avatarPhoto=demoAvatar(l.contactName);
...
function buildStyle(online){
  ...
  const sources={
    buildings:{type:"geojson",data:gj(merged.buildings),generateId:true},
  };
  const layers=[];
  if(online){
    ...
    layers.push(
      ...
      {id:"bld-2d",type:"fill",source:"buildings",minzoom:12.5,
        paint:{"fill-color":["case",
            ["boolean",["feature-state","hover"],false],"#e5484d",
            ["boolean",["feature-state","hasListing"],false],"#9061f9",
            ["interpolate",["linear"],["zoom"],13,"#eae6dd",16,"#e4e0d5"]],
          "fill-outline-color":"#d3cec1","fill-opacity":["interpolate",["linear"],["zoom"],12.5,0,13.2,1]}},
    );
  }
  layers.push({id:"bld-3d",type:"fill-extrusion",source:"buildings",minzoom:12.5,layout:{visibility:"none"},
    paint:{"fill-extrusion-color":["case",
        ...
        ["boolean",["feature-state","hasListing"],false],"#9061f9",
        "#d8d3c7"],
      "fill-extrusion-height":["get","h"],"fill-extrusion-opacity":.92}});
...
/* raw building footprints, same order as the "buildings" geojson source (so array index === generated feature id) */
let ALL_BUILDINGS=null;
function allBuildings(){
  if(!ALL_BUILDINGS){
    ALL_BUILDINGS=[];
    for(const k in GEO.cities) ALL_BUILDINGS.push(...GEO.cities[k].buildings.features);
  }
...
function createMap(container,online,opts={}){
  ...
  let hoveredBldId=null;
  const bldLayers=online?["bld-3d"]:["bld-3d","bld-2d"];
  const clearHover=()=>{
    if(hoveredBldId!=null){ map.setFeatureState({source:"buildings",id:hoveredBldId},{hover:false}); hoveredBldId=null; }
  };
  bldLayers.forEach(id=>{
    map.on("mousemove",id,e=>{
      if(!e.features.length)return;
      const fid=e.features[0].id;
      if(hoveredBldId===fid)return;
      clearHover();
      hoveredBldId=fid;
      map.setFeatureState({source:"buildings",id:fid},{hover:true});
      map.getCanvas().style.cursor="pointer";
    });
    map.on("mouseleave",id,()=>{ clearHover(); map.getCanvas().style.cursor=""; });
  });
  let highlightedIds=[];
  map.highlightListingBuildings=listings=>{
    if(!map.isStyleLoaded()){ map.once("load",()=>map.highlightListingBuildings(listings)); return; }
    const feats=allBuildings();
    highlightedIds.forEach(id=>map.setFeatureState({source:"buildings",id},{hasListing:false}));
    highlightedIds=[];
    (listings||[]).forEach(l=>{
      ...
      for(let idx=0;idx<feats.length;idx++){
        const f=feats[idx];
        const ring=f.geometry&&f.geometry.type==="Polygon"?f.geometry.coordinates[0]:null;
        if(!ring)continue;
        ...
        if(inside){ map.setFeatureState({source:"buildings",id:idx},{hasListing:true}); highlightedIds.push(idx); break; }
      }
    });
  };
...
const OSM_CATS={
  ...
  restaurants:{label:"Restorante",icon:"🍽️"},
  cafes:{label:"Kafene",icon:"☕"},
  nightlife:{label:"Jetë nate",icon:"🍸"},
  groceries:{label:"Ushqimore",icon:"🛒"},
  shopping:{label:"Pazar",icon:"🛍️"},
  entertainment:{label:"Argëtim",icon:"🎭"},
};
function classifyOsmTags(tags){
  ...
  if(["supermarket","convenience","grocery"].includes(tags.shop)) return "groceries";
  if(["mall","clothes","department_store","shoes","electronics"].includes(tags.shop)) return "shopping";
  if(["cinema","theatre"].includes(tags.amenity)||tags.tourism==="museum") return "entertainment";
  return null;
}
...
function scoreLabel(n){
  if(n>=90) return "Shkëlqyeshëm";
  if(n>=70) return "Shumë mirë";
  if(n>=50) return "Mesatare";
  if(n>=25) return "I kufizuar";
  return "Shumë i kufizuar";
}
...
/* ================= listing limits (plans/subscriptions disabled for now — everyone gets the free tier) & lead tracking ================= */
const FREE_LISTINGS_LIMIT=30, FREE_PHOTOS_LIMIT=20;
...
function searchMatches(params,l){
  return l.status==="published"
    ...
    &&(!params.priceMin||priceInEUR(l)>=params.priceMin)
    &&(!params.priceMax||priceInEUR(l)<=params.priceMax)
...
"Ecshmëria, transporti & çiklizmi":"Walkability, transit & bike","Zona përreth":"Areas of interest",
"Vlerësim i bazuar në të dhënat e hapura OpenStreetMap përreth kësaj prone (~1 km) — jo shërbimi zyrtar Walk Score®.":"Estimate based on open OpenStreetMap data around this property (~1 km) — not the official Walk Score® service.",
"Përdorni hartën për të parë çfarë ka rreth kësaj prone.":"Use the map to see what's around this property.",
"Nuk u gjetën vende në OpenStreetMap për këtë kategori pranë kësaj prone.":"No places found on OpenStreetMap for this category near this property.",
"Vlerësimi nuk u ngarkua dot. Provoni përsëri.":"The estimate couldn't load. Try again.",
"Nuk u mundësua lidhja me OpenStreetMap. Provoni përsëri më vonë.":"Couldn't connect to OpenStreetMap. Try again later.",
"Kërkohet lidhje interneti për vlerësimin dhe zonat përreth.":"An internet connection is required for the estimate and nearby areas.",
"Kërkohet lidhje interneti për të shfaqur zonat përreth.":"An internet connection is required to show nearby areas.",
"Kjo veçori kërkon që faqja të hapet përmes serverit (node server.js), jo si skedar i hapur drejtpërdrejt.":"This feature requires the site to be opened through the server (node server.js), not as a directly-opened file.",
"Kjo veçori kërkon serverin e faqes për të funksionuar.":"This feature requires the site's server to be running.",
"Highlights":"Highlights","Restorante":"Restaurants","Kafene":"Cafes","Jetë nate":"Nightlife",
"Ushqimore":"Groceries","Pazar":"Shopping","Argëtim":"Entertainment",
"Ecshmëria":"Walkability","Transporti":"Transit","Çiklizmi":"Biking",
"Shkëlqyeshëm":"Excellent","Shumë mirë":"Very good","Mesatare":"Average","I kufizuar":"Limited","Shumë i kufizuar":"Very limited",
...
"Rreth Nesh":"About Us",
" është platforma digjitale e pronave për të gjithë Shqipërinë.":" is the digital real estate platform for all of Albania.",
"Misioni ynë është i thjeshtë: të bëjmë blerjen, shitjen dhe qiradhënien e pronave, transparente dhe të lehtë. Çdo pronë vendoset në hartë interaktive 3D, me vendndodhje të saktë dhe pamje satelitore. Kështu, prona juaj bëhet më e lehtë për t'u gjetur dhe merr ekspozim maksimal.":"Our mission is simple: to make buying, selling, and renting properties transparent and easy. Every property is placed on an interactive 3D map, with an exact location and satellite view. This way, your property becomes easier to find and gets maximum exposure.",
...
"Krijoni llogarinë tuaj si agjent ose agjenci dhe menaxhoni të gjitha pronat tuaja në një vend të vetëm. Ndiqni performancën e çdo prone me statistika të detajuara mbi shikimet, interesimin dhe telefonatat, ndërsa promovoni pronat tuaja në mënyrë fleksibël, sipas buxhetit dhe nevojave tuaja.":"Create your account as an agent or agency and manage all your properties in one place. Track each property's performance with detailed statistics on views, interest, and calls, while promoting your properties flexibly, according to your budget and needs.",
...
"çdo pronar, agjent apo agjenci mund të publikojë pronat pa pagesë.":"any owner, agent, or agency can publish their properties for free.",
"Kontakt i drejtpërdrejtë":"Direct contact",
"blerësit dhe qiramarrësit kontaktojnë direkt me pronarin ose agjentin, nëpërmjet numrit të telefonit ose WhatsApp-it, pa ndërmjetës.":"buyers and tenants contact the owner or agent directly, via phone number or WhatsApp, with no middleman.",
"Kërkim i avancuar dhe kërkime të ruajtura":"Advanced search and saved searches",
"filtroni sipas çmimit, sipërfaqes, dhomave dhe veçorive, ruani kërkimin dhe merrni njoftim kur shtohen prona që përputhen me kërkesat tuaja.":"filter by price, area, rooms and features, save your search, and get notified when properties matching your requirements are added.",
"Favoritet":"Favorites",
"ruani pronat që ju pëlqejnë me një klikim dhe rikthehuni për ti rishikuar kur të dëshironi.":"save the properties you like with one click and come back to review them whenever you want.",
"Promovim me ofertë ditore":"Promotion with a daily bid",
"kush dëshiron më shumë dukshmëri, promovoni pronën tuaj nëpërmjet një oferte ditore në mënyrë që të maksimizoni shikueshmërinë dhe të rendisni më lartë në renditje pronën tuaj.":"if you want more visibility, promote your property with a daily bid to maximize its visibility and rank it higher in the listings.",
"Pagesa fleksibël":"Flexible payments",
"rimbushni bilancin me PayPal ose me anë të kriptomonedhave (BTC, ETH, USDT, SOL).":"top up your balance with PayPal or with cryptocurrency (BTC, ETH, USDT, SOL).",
"Besueshmëri dhe siguri":"Trust and security",
"Siguria dhe transparenca janë prioriteti ynë. Llogaritë verifikohen përmes email-it, ndërsa të dhënat dhe fjalëkalimet mbrohen me masa të sigurta enkriptimi. Pagesat për shërbimet e promovimit kryhen përmes ofruesve të besueshëm dhe të sigurt, si PayPal dhe Coinbase Commerce.":"Security and transparency are our priority. Accounts are verified via email, while data and passwords are protected with secure encryption measures. Payments for promotion services are processed through trusted and secure providers, such as PayPal and Coinbase Commerce.",
"Prona është një platformë ndërmjetësimi dhe nuk garanton vlefshmërinë e pronave të listuara. Përpara çdo pagese ose transaksioni, ju rekomandojmë të verifikoni pronën, pronësinë dhe dokumentacionin përkatës.":"Prona is an intermediary platform and does not guarantee the validity of listed properties. Before any payment or transaction, we recommend you verify the property, ownership, and relevant documentation.",
...
"Partnerët":"Partners",
"Specializimi: Të gjithë":"Specialization: All","Vetëm Agjentë":"Agents only","Vetëm Agjenci":"Agencies only",
"Renditja: Më i rekomanduari":"Sort: Most recommended","Publikimi më i ri":"Newest listing","Emri (A–Z)":"Name (A–Z)",
"Bëhuni Partner":"Become a Partner","Publikoni pronat tuaja dhe dukuni te ky drejtori, falas.":"Publish your properties and appear in this directory, for free.",
"Të gjitha llojet":"All types","Të gjitha qytetet":"All cities",
"Dhoma: Të gjitha":"Rooms: All",
"Renditja: Më të rejat":"Sort: Newest",
"Çmimi min (€)":"Min price (€)","Çmimi max (€)":"Max price (€)","Sipërfaqja min (m²)":"Min area (m²)","Sipërfaqja max (m²)":"Max area (m²)",
"Asnjë pronë nuk përputhet me filtrat.":"No properties match these filters.",
"🚩 Raporto këtë shpallje":"🚩 Report this listing","Raporto shpalljen":"Report listing",
"E rreme / nuk ekziston":"Fake / doesn't exist","Është shitur ose dhënë me qira":"Already sold or rented",
"Informacion i gabuar":"Incorrect information","Spam ose reklamë":"Spam or advertisement","Tjetër":"Other",
"Arsyeja":"Reason","Detaje (opsionale)":"Details (optional)","Dërgo raportimin":"Send report",
"Raportime të hapura":"Open reports","Shpërfill":"Dismiss","Fshi shpalljen":"Delete listing",
"E rreme":"Fake","E shitur/dhënë":"Sold/rented","Info e gabuar":"Wrong info","Spam":"Spam",
"anonim":"anonymous",
"Gjeni":"Find your","partnerin":"partner","tuaj në Prona":"on Prona",
"Agjentë dhe agjenci — publikojnë prona reale, me kontakt të drejtpërdrejtë me telefon ose WhatsApp.":"Agents and agencies — publishing real properties, with direct contact by phone or WhatsApp.",
"Kërko emër ose qytet…":"Search name or city…",
"pronë aktive":"active property","prona aktive":"active properties",
"Shiko pronat":"View properties",
"Ende asnjë agjent apo agjenci me shpallje aktive.":"No agents or agencies with active listings yet.",
"Shiko profilin":"View profile","Partneri nuk u gjet":"Partner not found",
"Ky partner mund të mos ketë më shpallje aktive.":"This partner may no longer have active listings.",
"Kthehu te Partnerët":"Back to Partners",
"Prona aktive":"Active properties","Shikime gjithsej":"Total views","Telefonata të shfaqura":"Phone reveals shown","Klikime WhatsApp":"WhatsApp clicks",
"Portofoli":"Portfolio","Gama e çmimeve:":"Price range:",
"Zonat e shërbimit":"Service areas","Prona në hartë":"Properties on the map",
"Pronat aktive":"Active properties",
...
function initCookieBanner(){
  if(Store.read("prona_cookie_ok",null)) return;
  const el=document.createElement("div");
  el.className="cookie-banner";
  el.innerHTML=`
    <p>${T("Ky sajt përdor cookie thelbësore për të funksionuar — për hyrjen në llogari dhe për të mbajtur mend preferencat tuaja (gjuha, tema).","This site uses essential cookies to work — for account login and to remember your preferences (language, theme).")}
      <a href="#/privacy">${T("Mëso më shumë","Learn more")}</a></p>
    <button class="btn primary" id="cookieOkBtn" type="button">${T("E kuptova","Got it")}</button>`;
  document.body.appendChild(el);
  translateDOM(el);
  $("#cookieOkBtn",el).addEventListener("click",()=>{
    Store.write("prona_cookie_ok","1");
    el.remove();
  });
}
  ...
  return `<footer class="site-footer">
    <div class="foot-grid">
      ...
      <div class="foot-col"><b>Informacion</b>
        <button type="button" data-foot-go="#/partners">Partnerët</button>
...
function renderHeader(){
  const u=currentUser();
  $("#header").innerHTML=`
    <button class="logo" data-go="#/"><svg class="mark" viewBox="0 0 96 96" aria-hidden="true"><rect width="96" height="96" rx="22" fill="var(--accent)"/><path d="M26 52 L48 30 L70 52" fill="none" stroke="#fff" stroke-width="9" stroke-linecap="round" stroke-linejoin="round"/><circle cx="48" cy="62" r="8" fill="#fff"/></svg><b>prona</b></button>
    <nav class="hnav" aria-label="Kryesore">
      ...
      <button data-page="#/partners" ${location.hash.startsWith("#/partners")?'class="current"':""}>Partnerët</button>
    ...
    <div class="header-right">
      ...
      <button class="btn primary" data-go="#/add">+ <span>Shto pronë</span></button>
      ${u?`<button class="icon-btn" data-go="#/favorites" title="Të preferuarat" aria-label="Të preferuarat">♥${getFavs().length?`<span class="icon-count">${getFavs().length}</span>`:""}</button>
           ...
           <div class="account-menu" id="acctMenu">
             <button class="user-chip" data-go="#/my"><span class="avatar">${esc(u.name[0].toUpperCase())}</span><span class="user-chip-name">${esc(u.name.split(" ")[0])}</span></button>
             <button class="acct-more-btn" id="acctMoreBtn" type="button" aria-label="Më shumë">▾</button>
  ...
  const am=$("#acctMenu");
  const amb=$("#acctMoreBtn");
  if(amb) amb.addEventListener("click",e=>{ e.preventDefault(); e.stopPropagation(); am.classList.toggle("open"); });
  if(am) document.addEventListener("click",e=>{ if(!am.contains(e.target)) am.classList.remove("open"); });
...
/* ================= verify & reset modals ================= */
function reportModal(listingId){
  if(!Remote.enabled){ toast(T("Raportimi kërkon që faqja të hapet përmes serverit.","Reporting requires the site to be running through the server.")); return; }
  const root=$("#modal-root");
  const REASONS=[["fake","E rreme / nuk ekziston"],["sold","Është shitur ose dhënë me qira"],["wrong_info","Informacion i gabuar"],["spam","Spam ose reklamë"],["other","Tjetër"]];
  root.innerHTML=`<div class="modal-back"><div class="modal modal-wrap">
    <button class="close-x" aria-label="Mbyll">×</button>
    <h2>Raporto shpalljen</h2>
    <p class="lede">${T("Na tregoni pse kjo shpallje duket problematike. Administratorët do ta shqyrtojnë.","Tell us why this listing looks problematic. Admins will review it.")}</p>
    <form id="rForm">
      <div class="frow"><label for="r-reason">Arsyeja</label>
        <select id="r-reason">${REASONS.map(([k,label])=>`<option value="${k}">${label}</option>`).join("")}</select></div>
      <div class="frow"><label for="r-note">Detaje (opsionale)</label><textarea id="r-note" rows="3" placeholder="${T("Shpjegoni shkurt…","Briefly explain…")}"></textarea></div>
      <button class="btn primary" style="width:100%;justify-content:center" type="submit">Dërgo raportimin</button>
    </form>
  </div></div>`;
  translateDOM(root);
  const close=()=>root.innerHTML="";
  ...
  $("#rForm",root).addEventListener("submit",async e=>{
    e.preventDefault();
    try{
      const j=await apiCall("/api/report","POST",{listingId,reason:$("#r-reason",root).value,note:$("#r-note",root).value.trim()});
      close();
      toast(j.alreadyReported?T("E keni raportuar tashmë këtë shpallje.","You've already reported this listing."):T("Faleminderit — raportimi u dërgua.","Thanks — your report was sent."));
...
function destroyMap(){
  ...
  if(areaMap){try{areaMap.remove();}catch(e){}areaMap=null;}
}
...
async function viewListing(){
  ...
  const priceLabel=l=>{
    const sym=l.currency==="ALL"?x=>`${fmt(x)} L`:x=>`€${fmt(x)}`;
    return l.dealType==="sale"?sym(l.price):l.dealType==="rent"?`${sym(l.price)}/muaj`:`${sym(l.price)}/natë`;
  };
  const matchesQ=l=>{
    if(!q) return true;
  ...
  const visible=()=>allListings().filter(l=>l.status==="published"&&l.dealType===deal
      ...
      &&(!priceMin||priceInEUR(l)>=priceMin)&&(!priceMax||priceInEUR(l)<=priceMax)
  ...
  function rebuildPins(){
    ...
    updatePinVis();
    map.highlightListingBuildings(visible());
  }
  ...
  function updatePinVis(){
    ...
    for(const key in cityPins){
      ...
      if(!cityMode&&list){
        ...
        el.innerHTML=`<b>${CITIES[key].name}</b><span>${list.length} shpallje · nga ${priceLabel(cheapest)}</span>`;
        el.style.display="";
  ...
  function renderCards(){
    ...
    $("#listSub").textContent=vis.length?`nga ${priceLabel(vis.reduce((a,b)=>a.price<b.price?a:b))}`:"";
    ...
    vis.forEach(l=>{
      ...
      el.innerHTML=`<span class="thumb-wrap"><img class="thumb" loading="lazy" src="${l.photos[0]||svgThumb(1)}" alt="">
        ...
        <span class="card-info"><span class="price">${priceLabel(l)} ${l.dealType==="sale"&&l.totalArea?`<small>${l.currency==="ALL"?fmt(l.price/l.totalArea)+" L":"€"+fmt(l.price/l.totalArea)}/m²</small>`:""}</span>
...
async function viewDetail(id){
  ...
  const online=await checkTiles();
  const sym=l.currency==="ALL"?x=>`${fmt(x)} L`:x=>`€${fmt(x)}`;
  const priceLabel=l.dealType==="sale"?sym(l.price):l.dealType==="rent"?`${sym(l.price)} / muaj`:`${sym(l.price)} / natë`;
  ...
  v.innerHTML=`<div class="detail">
    ...
    <div class="detail-grid">
      <div>
        ...
        <h2 class="section-title">Ecshmëria, transporti &amp; çiklizmi</h2>
        <div class="panel-box" id="scoreBox">
          <div class="score-row" id="scoreRow">
            <div class="score-badge loading"><span class="score-spin"></span><b>Ecshmëria</b></div>
            <div class="score-badge loading"><span class="score-spin"></span><b>Transporti</b></div>
            <div class="score-badge loading"><span class="score-spin"></span><b>Çiklizmi</b></div>
          </div>
          <p class="score-note">Vlerësim i bazuar në të dhënat e hapura OpenStreetMap përreth kësaj prone (~1 km) — jo shërbimi zyrtar Walk Score®.</p>
        </div>
        <h2 class="section-title">Zona përreth</h2>
        <div class="panel-box">
          <p style="font-size:12.5px;color:var(--ink-soft);margin:0 0 10px">Përdorni hartën për të parë çfarë ka rreth kësaj prone.</p>
          <div class="mini-map" id="areaMapWrap"><div class="mapbox" id="areaMap"></div></div>
          <div class="area-tabs" id="areaTabs">
            ${Object.entries(OSM_CATS).map(([k,c],i)=>`<button type="button" class="area-tab${i===0?" on":""}" data-cat="${k}">${c.icon} <span>${c.label}</span></button>`).join("")}
          </div>
          <p class="area-note" id="areaNote" hidden>Nuk u gjetën vende në OpenStreetMap për këtë kategori pranë kësaj prone.</p>
        </div>
      </div>
      <div class="detail-side">
        <div class="panel-box">
          ...
          <h2>${esc(l.title)}</h2>
          <div class="big-price">${priceLabel} ${l.dealType==="sale"&&l.totalArea?`<small>· ${sym(l.price/l.totalArea)}/m²</small>`:""}</div>
        ...
        <div class="panel-box">
          ...
          <div class="contact-row">
            ${avatarHTML(l.contactName||"P",l.avatarPhoto)}
          ...
          <button class="report-link" id="reportBtn" type="button">🚩 Raporto këtë shpallje</button>
        </div>
  ...
  $("#reportBtn").addEventListener("click",()=>reportModal(l.id));
  /* photo lightbox */
  ...
  const area=areaMap=createMap("areaMap",online,{center:[l.lng,l.lat],zoom:14.5,nav:false});
  const centerEl=document.createElement("div"); centerEl.className="draft-pin"; centerEl.style.cursor="default";
  new maplibregl.Marker({element:centerEl,anchor:"bottom"}).setLngLat([l.lng,l.lat]).addTo(area);
  let areaMarkers=[]; const clearAreaMarkers=()=>{areaMarkers.forEach(m=>m.remove());areaMarkers=[];};
  const areaNote=$("#areaNote",v), scoreRow=$("#scoreRow",v);
  let areaDataCache=null;
  const renderCat=catKey=>{
    if(!areaDataCache)return;
    clearAreaMarkers();
    const list=catKey==="highlights"?areaDataCache.places:areaDataCache.places.filter(p=>p.cat===catKey);
    areaNote.hidden=!!list.length;
    list.slice(0,40).forEach(p=>{
      const pinEl=document.createElement("div"); pinEl.className="area-pin"; pinEl.textContent=OSM_CATS[p.cat].icon; pinEl.title=p.name||OSM_CATS[p.cat].label;
      areaMarkers.push(new maplibregl.Marker({element:pinEl}).setLngLat([p.lng,p.lat]).addTo(area));
    });
  };
  if(online&&Remote.enabled){
    fetchAreaData(l.lat,l.lng).then(data=>{
      if(!data){
        scoreRow.innerHTML=`<p class="score-note" style="margin:0">Vlerësimi nuk u ngarkua dot. Provoni përsëri.</p>`;
        areaNote.hidden=false; areaNote.textContent="Nuk u mundësua lidhja me OpenStreetMap. Provoni përsëri më vonë.";
        return;
      }
      areaDataCache=data;
      scoreRow.innerHTML=`
        <div class="score-badge"><b>${data.walkScore}</b><span>Ecshmëria</span><i>${scoreLabel(data.walkScore)}</i></div>
        <div class="score-badge"><b>${data.transitScore}</b><span>Transporti</span><i>${scoreLabel(data.transitScore)}</i></div>
        <div class="score-badge"><b>${data.bikeScore}</b><span>Çiklizmi</span><i>${scoreLabel(data.bikeScore)}</i></div>`;
      renderCat("highlights");
    });
  } else if(!online){
    scoreRow.innerHTML=`<p class="score-note" style="margin:0">Kërkohet lidhje interneti për vlerësimin dhe zonat përreth.</p>`;
    areaNote.hidden=false; areaNote.textContent="Kërkohet lidhje interneti për të shfaqur zonat përreth.";
  } else {
    scoreRow.innerHTML=`<p class="score-note" style="margin:0">Kjo veçori kërkon që faqja të hapet përmes serverit (node server.js), jo si skedar i hapur drejtpërdrejt.</p>`;
    areaNote.hidden=false; areaNote.textContent="Kjo veçori kërkon serverin e faqes për të funksionuar.";
  }
  $$("#areaTabs .area-tab",v).forEach(b=>b.addEventListener("click",()=>{
    $$("#areaTabs .area-tab",v).forEach(x=>x.classList.toggle("on",x===b));
...
async function viewAdd(editId){
  ...
  const photoLimit=FREE_PHOTOS_LIMIT;
  const renderPhotos=()=>{
  ...
  async function save(status){
    ...
    if(Remote.enabled){
      ...
      renderHeader();
      return {...j.listing,_duplicateWarning:j.duplicateWarning};
    }
  ...
  $("#publishBtn").addEventListener("click",async()=>{
    ...
    try{
      ...
      if(rec._duplicateWarning) toast(T("Publikuar — kjo pronë duket e ngjashme me një shpallje ekzistuese, administratorët do ta shqyrtojnë.","Published — this listing looks similar to an existing one, admins will review it."));
      else toast(editing?"Shpallja u përditësua":"Shpallja u publikua");
...
async function viewAdmin(){
  ...
  let reports=[];
  if(Remote.enabled){
    try{ reports=(await apiCall("/api/admin/reports")).reports; }catch(e){/* non-fatal */}
  }
  ...
  v.innerHTML=`<div class="myprops"><div class="inner" style="max-width:1000px">
    ...
    <div class="admin-stats">
      ...
      <div class="stat"><span>Raportime të hapura</span><b>${data.reportsOpenCount}</b></div>
      <div class="stat"><span>Të ardhura nga promovimi</span><b>€${fmt(data.revenue)}</b></div>
    </div>
    ${reports.length?`<h2 class="section-title">${T(`Raportimet e hapura (${reports.length})`,`Open reports (${reports.length})`)}</h2>
  ...
  const rrows=$("#admReports");
  if(rrows){
    reports.forEach(r=>{
      const row=document.createElement("div"); row.className="prop-row";
      row.innerHTML=`<div class="info" style="padding-left:6px"><b>${esc(r.reasonLabel)}</b> <span class="tag draft">${esc(r.listingTitle)}</span>
        <span>${r.note?esc(r.note)+" · ":""}${r.reporterEmail?esc(r.reporterEmail):"anonim"} · ${new Date(r.createdAt).toLocaleDateString("sq-AL")}</span></div>
        <div class="actions">
          ${r.listingStatus?`<button class="btn" data-a="view">Shiko</button>`:""}
          <button class="btn ghost" data-a="dismiss">Shpërfill</button>
          ${r.listingStatus?`<button class="btn ghost" data-a="del">Fshi shpalljen</button>`:""}
        </div>`;
      const vb=row.querySelector('[data-a="view"]'); if(vb)vb.addEventListener("click",()=>location.hash="#/property/"+r.listingId);
      row.querySelector('[data-a="dismiss"]').addEventListener("click",async()=>{
        try{ await apiCall("/api/admin/reports/"+r.id+"/resolve","POST",{}); toast(T("U përditësua","Updated")); viewAdmin(); }
        catch(err){toast(err.message);}
      });
      const db_=row.querySelector('[data-a="del"]'); if(db_)db_.addEventListener("click",async()=>{
        if(!confirm(T("Ta fshini këtë shpallje si administrator?","Delete this listing as administrator?")))return;
        try{ await apiCall("/api/admin/listings/"+r.listingId,"DELETE"); Remote.listings=Remote.listings.filter(x=>x.id!==r.listingId); toast(T("Shpallja u fshi","Listing deleted")); viewAdmin(); }
        catch(err){toast(err.message);}
      });
  ...
  userListings.forEach(l=>{
    ...
    row.innerHTML=`<img loading="lazy" src="${l.photos[0]||svgThumb(2)}" alt="">
      <div class="info"><b>${esc(l.title)}</b> ${(l.possibleDuplicateOf||[]).length?`<span class="tag draft">${T("DUBLIKATË E MUNDSHME","POSSIBLE DUPLICATE")}</span>`:""}<span>${esc(l.owner||"")} · ${CITIES[l.city]?CITIES[l.city].name:l.city} · €${fmt(l.price)} · ${l.status}</span></div>
...
function viewMy(){
  ...
  v.innerHTML=`<div class="myprops"><div class="inner">
    ...
    <p style="font-size:12.5px;margin:0 0 18px">${T(`${mine.filter(l=>l.status==="published").length}/${FREE_LISTINGS_LIMIT} shpallje aktive`,`${mine.filter(l=>l.status==="published").length}/${FREE_LISTINGS_LIMIT} active listings`)}</p>
    <div id="rows"></div>
    ...
    row.innerHTML=`<img src="${l.photos[0]||svgThumb(3)}" alt="">
      <div class="info"><b>${esc(l.title)}</b>
        ...
        <span class="lead-stats" title="${T("Shikime · Telefonata të shfaqura · Klikime WhatsApp","Views · Phone reveals · WhatsApp clicks")}">👁 ${fmt(s.v||0)} · 📞 ${fmt(s.p||0)} · 💬 ${fmt(s.w||0)}
          <button class="stats-toggle" type="button" data-stats="${l.id}">${T("statistika ↓","stats ↓")}</button></span>
...
function getPartners(){
  const listings=allListings().filter(l=>l.status==="published"&&(l.accountType==="agent"||l.accountType==="agency"));
  const groups={};
  listings.forEach(l=>{
    const key=(l.phone||l.contactName||"").replace(/\D/g,"")||l.contactName;
    if(!groups[key]) groups[key]={key,name:l.contactName,phone:l.phone,whatsapp:false,accountType:l.accountType,cities:new Set(),dealTypes:new Set(),count:0,listings:[],avatarPhoto:l.avatarPhoto,latestAt:0};
    const g=groups[key];
    if(l.accountType==="agency") g.accountType="agency";
    if(l.whatsapp) g.whatsapp=true;
    if(!g.avatarPhoto&&l.avatarPhoto) g.avatarPhoto=l.avatarPhoto;
    g.cities.add(CITIES[l.city].name);
    g.dealTypes.add(l.dealType);
    g.count++;
    g.listings.push(l);
    if((l.createdAt||0)>g.latestAt) g.latestAt=l.createdAt||0;
  });
  return Object.values(groups).sort((a,b)=>b.count-a.count);
}
function dealTypesLabel(dealTypesSet){
  const order=["sale","rent","daily"]; const names={sale:"Shitje",rent:"Qira",daily:"Ditore"};
  return order.filter(k=>dealTypesSet.has(k)).map(k=>names[k]).join(" & ");
}
function viewPartners(){
  destroyMap();
  const v=$("#view");
  const partners=getPartners();
  v.innerHTML=`<div class="static-page partners-page"><div class="inner" style="max-width:1040px">
    <div class="partners-hero">
      <h1>Gjeni <b>partnerin</b> tuaj në Prona</h1>
      <p>Agjentë dhe agjenci — publikojnë prona reale, me kontakt të drejtpërdrejtë me telefon ose WhatsApp.</p>
      <div class="search-box" style="max-width:360px;margin:16px auto 0"><input id="pSearch" type="search" placeholder="Kërko emër ose qytet…" aria-label="Kërko partner"></div>
    </div>
    <div class="partners-toolbar">
      <span class="pcount" id="pCount"></span>
      <div class="partners-controls">
        <select class="sel" id="pSpec"><option value="">Specializimi: Të gjithë</option><option value="agent">Vetëm Agjentë</option><option value="agency">Vetëm Agjenci</option></select>
        <select class="sel" id="pSort">
          <option value="rec">Renditja: Më i rekomanduari</option>
          <option value="new">Publikimi më i ri</option>
          <option value="az">Emri (A–Z)</option>
        </select>
      </div>
    </div>
    <div class="partners-grid" id="partnersGrid"></div>
    ${!partners.length?`<p class="empty">Ende asnjë agjent apo agjenci me shpallje aktive.</p>`:""}
  </div></div>`;
  attachFooter($(".static-page .inner",v));
  const grid=$("#partnersGrid"), countEl=$("#pCount");
  const draw=()=>{
    const qq=($("#pSearch").value||"").toLowerCase().trim();
    const spec=$("#pSpec").value;
    const sortKey=$("#pSort").value;
    let list=partners.filter(p=>
      (!qq||p.name.toLowerCase().includes(qq)||[...p.cities].some(c=>c.toLowerCase().includes(qq)))
      &&(!spec||p.accountType===spec));
    if(sortKey==="new") list=[...list].sort((a,b)=>b.latestAt-a.latestAt);
    else if(sortKey==="az") list=[...list].sort((a,b)=>a.name.localeCompare(b.name));
    else list=[...list].sort((a,b)=>b.count-a.count);
    countEl.textContent=T(`${list.length} ${list.length===1?"partner":"partnerë"}`,`${list.length} ${list.length===1?"partner":"partners"}`);
    grid.innerHTML=list.map(p=>{
      const cities=[...p.cities]; const cityLabel=cities.length>1?`${cities[0]} +${cities.length-1}`:cities[0]||"";
      const dealsLabel=dealTypesLabel(p.dealTypes);
      return `<div class="partner-card">
        ${avatarHTML(p.name,p.avatarPhoto,"partner-avatar")}
        <b class="partner-name">${esc(p.name)}</b>
        <span class="partner-sub">${p.accountType==="agency"?"Agjenci":"Agjent imobiliar"} · ${esc(cityLabel)}</span>
        <span class="partner-stat">${p.count} ${p.count===1?"pronë":"prona"}${dealsLabel?" · "+esc(dealsLabel):""}</span>
        ${p.latestAt?`<span class="partner-latest">${T("Publikimi më i fundit","Latest listing")}: ${new Date(p.latestAt).toLocaleDateString("sq-AL")}</span>`:""}
        <button class="btn primary" style="width:100%;justify-content:center" data-partner-key="${esc(p.key)}">Shiko profilin</button>
      </div>`;
    }).join("")+`
      <div class="partner-card partner-cta">
        <div class="partner-cta-icon">★</div>
        <b class="partner-name">Bëhuni Partner</b>
        <span class="partner-sub">Publikoni pronat tuaja dhe dukuni te ky drejtori, falas.</span>
        <button class="btn" style="width:100%;justify-content:center" data-go="#/add">Shto pronën tuaj</button>
      </div>`;
    $$("[data-partner-key]",grid).forEach(b=>b.addEventListener("click",()=>{
      location.hash="#/partner/"+encodeURIComponent(b.dataset.partnerKey);
    }));
    $$("[data-go]",grid).forEach(b=>b.addEventListener("click",()=>location.hash=b.dataset.go));
  };
  draw();
  $("#pSearch").addEventListener("input",draw);
  $("#pSpec").addEventListener("change",draw);
  $("#pSort").addEventListener("change",draw);
}
async function viewPartnerProfile(key){
  const online=await checkTiles();
  destroyMap();
  const v=$("#view");
  const partners=getPartners();
  ...
  if(!p){
    v.innerHTML=`<div class="static-page"><div class="inner">
      <h1>Partneri nuk u gjet</h1>
      <p>Ky partner mund të mos ketë më shpallje aktive.</p>
      <button class="btn" data-go="#/partners">← Kthehu te Partnerët</button>
    </div></div>`;
    $$("[data-go]",v).forEach(b=>b.addEventListener("click",()=>location.hash=b.dataset.go));
    attachFooter($(".static-page .inner",v));
    return;
  ...
  const dealCounts={sale:0,rent:0,daily:0};
  let totalV=0,totalP=0,totalW=0,minPrice=Infinity,maxPrice=0;
  p.listings.forEach(l=>{
    dealCounts[l.dealType]=(dealCounts[l.dealType]||0)+1;
    const s=l.stats||{}; totalV+=s.v||0; totalP+=s.p||0; totalW+=s.w||0;
    if(l.price){ minPrice=Math.min(minPrice,l.price); maxPrice=Math.max(maxPrice,l.price); }
  });
  v.innerHTML=`<div class="static-page partner-profile-page"><div class="inner" style="max-width:960px">
    <button class="btn ghost" data-go="#/partners" style="margin-bottom:16px">← Kthehu te Partnerët</button>
    <div class="pp-header">
      ${avatarHTML(p.name,p.avatarPhoto,"pp-avatar")}
      <div class="pp-head-info">
        <h1>${esc(p.name)}</h1>
        <p>${p.accountType==="agency"?"Agjenci":"Agjent imobiliar"} · ${esc(cities.join(", "))}</p>
      </div>
    </div>
    <div class="pp-stats">
      <div class="pp-stat"><b>${p.count}</b><span>Prona aktive</span></div>
      <div class="pp-stat"><b>${fmt(totalV)}</b><span>Shikime gjithsej</span></div>
      <div class="pp-stat"><b>${fmt(totalP)}</b><span>Telefonata të shfaqura</span></div>
      <div class="pp-stat"><b>${fmt(totalW)}</b><span>Klikime WhatsApp</span></div>
    </div>
    <div class="pp-grid">
      <div class="pp-main">
        <div class="panel-box">
          <b style="font-size:13.5px">Portofoli</b>
          <div class="pp-deal-bars">
            ${["sale","rent","daily"].map(k=>dealCounts[k]?`<div class="pp-deal-row"><span>${DEALS[k]}</span><div class="pp-bar"><div style="width:${Math.round(dealCounts[k]/p.count*100)}%"></div></div><b>${dealCounts[k]}</b></div>`:"").join("")}
          </div>
          ${minPrice<Infinity?`<p style="font-size:12.5px;color:var(--ink-soft);margin-top:10px">Gama e çmimeve: <b>€${fmt(minPrice)} – €${fmt(maxPrice)}</b></p>`:""}
        </div>
        <div class="panel-box">
          <b style="font-size:13.5px">Zonat e shërbimit</b>
          <div class="feat-chips" style="margin-top:10px">${cities.map(c=>`<button type="button" class="tag city" data-pcity="${Object.keys(CITIES).find(k=>CITIES[k].name===c)}" style="cursor:pointer;border:0">${esc(c)}</button>`).join("")}</div>
        </div>
        <div class="panel-box">
          <b style="font-size:13.5px">Prona në hartë</b>
          <div class="mini-map" style="height:260px;margin-top:10px"><div class="mapbox" id="partnerMap"></div></div>
        </div>
        <h2 class="section-title">${T(`Pronat e ${p.name}`,`Properties by ${p.name}`)}</h2>
        <div class="panel-box pp-filters">
          <div class="seg" id="ppDealSeg" role="group" aria-label="Lloji i marrëveshjes">
            ${["sale","rent","daily"].filter(k=>dealCounts[k]).map((k,i)=>`<button type="button" data-v="${k}" class="${i===0?"on":""}">${T(`${k==="sale"?"Blej":k==="rent"?"Qira":"Ditore"} (${dealCounts[k]})`,`${k==="sale"?"Buy":k==="rent"?"Rent":"Daily"} (${dealCounts[k]})`)}</button>`).join("")}
          </div>
          <div class="pp-filter-row">
            <select class="sel" id="ppType"><option value="">Të gjitha llojet</option>${[...new Set(p.listings.map(l=>l.propertyType))].map(k=>`<option value="${k}">${PTYPES[k]}</option>`).join("")}</select>
            ${cities.length>1?`<select class="sel" id="ppCity"><option value="">Të gjitha qytetet</option>${cities.map(c=>`<option value="${Object.keys(CITIES).find(k=>CITIES[k].name===c)}">${esc(c)}</option>`).join("")}</select>`:""}
            <select class="sel" id="ppBeds"><option value="0">Dhoma: Të gjitha</option><option value="1">1+</option><option value="2">2+</option><option value="3">3+</option><option value="4">4+</option></select>
            <select class="sel" id="ppSort">
              <option value="new">Renditja: Më të rejat</option>
              <option value="price-asc">Çmimi ↑</option>
              <option value="price-desc">Çmimi ↓</option>
              <option value="area-desc">Sipërfaqja ↓</option>
            </select>
          </div>
          <div class="pp-filter-row">
            <input class="sel" id="ppPriceMin" type="number" min="0" placeholder="Çmimi min (€)">
            <input class="sel" id="ppPriceMax" type="number" min="0" placeholder="Çmimi max (€)">
            <input class="sel" id="ppAreaMin" type="number" min="0" placeholder="Sipërfaqja min (m²)">
            <input class="sel" id="ppAreaMax" type="number" min="0" placeholder="Sipërfaqja max (m²)">
            <button class="btn ghost" id="ppClear" type="button">Pastro filtrat</button>
          </div>
          <p class="pp-filter-count" id="ppFilterCount"></p>
        </div>
        <div class="pp-listings" id="ppListings"></div>
      </div>
      <div class="pp-side">
        <div class="panel-box">
          <b style="font-size:13.5px">Kontakti</b>
          <div class="contact-row" style="margin-top:10px">
            ${avatarHTML(p.name,p.avatarPhoto)}
            <span class="who"><b>${esc(p.name)}</b><span>${p.accountType==="agency"?"Agjenci":"Agjent imobiliar"}</span></span>
          </div>
          <div style="display:flex;gap:8px;margin-top:14px">
            ${p.phone?`<a class="btn primary" style="flex:1;justify-content:center" href="tel:${esc(p.phone)}">Telefon</a>`:""}
            ${p.whatsapp&&p.phone?`<a class="btn" style="flex:1;justify-content:center" href="https://wa.me/${esc(p.phone.replace(/\D/g,""))}" target="_blank" rel="noopener">WhatsApp</a>`:""}
          </div>
        </div>
      </div>
    </div>
  ...
  $$("[data-go]",v).forEach(b=>b.addEventListener("click",()=>location.hash=b.dataset.go));
  $$("[data-pcity]",v).forEach(b=>b.addEventListener("click",()=>{
    ROUTE_DEAL="sale"; location.hash="#/"; render();
    setTimeout(()=>{const s=$("#citySel"); if(s){s.value=b.dataset.pcity; s.dispatchEvent(new Event("change"));}},60);
  }));
  attachFooter($(".static-page .inner",v));
  const listEl=$("#ppListings"), filterCountEl=$("#ppFilterCount");
  const defaultDeal=["sale","rent","daily"].find(k=>dealCounts[k])||"sale";
  const pFilters={deal:defaultDeal};
  const drawListings=()=>{
    const ptype=$("#ppType").value, cityF=$("#ppCity",v)?$("#ppCity").value:"", beds=+$("#ppBeds").value,
      priceMin=+$("#ppPriceMin").value||0, priceMax=+$("#ppPriceMax").value||Infinity,
      areaMin=+$("#ppAreaMin").value||0, areaMax=+$("#ppAreaMax").value||Infinity, sortKey=$("#ppSort").value;
    let list=p.listings.filter(l=>l.dealType===pFilters.deal
      &&(!ptype||l.propertyType===ptype)&&(!cityF||l.city===cityF)&&(!beds||l.bedrooms>=beds)
      &&priceInEUR(l)>=priceMin&&priceInEUR(l)<=priceMax&&l.totalArea>=areaMin&&l.totalArea<=areaMax);
    if(sortKey==="price-asc") list=[...list].sort((a,b)=>a.price-b.price);
    else if(sortKey==="price-desc") list=[...list].sort((a,b)=>b.price-a.price);
    else if(sortKey==="area-desc") list=[...list].sort((a,b)=>b.totalArea-a.totalArea);
    else list=[...list].sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));
    filterCountEl.textContent=T(`${list.length} ${list.length===1?"pronë":"prona"}`,`${list.length} ${list.length===1?"property":"properties"}`);
    listEl.innerHTML=list.length?list.map(l=>`
      <div class="card" data-id="${l.id}" role="button" tabindex="0">
        <span class="thumb-wrap"><img class="thumb" loading="lazy" src="${l.photos[0]||svgThumb(1)}" alt=""></span>
        <span class="card-info"><span class="price">${l.currency==="ALL"?(l.dealType==="sale"?`${fmt(l.price)} L`:l.dealType==="rent"?`${fmt(l.price)} L/muaj`:`${fmt(l.price)} L/natë`):(l.dealType==="sale"?`€${fmt(l.price)}`:l.dealType==="rent"?`€${fmt(l.price)}/muaj`:`€${fmt(l.price)}/natë`)}</span>
        <span class="name">${esc(l.title)}</span>
        <span class="tags"><span class="tag city">${CITIES[l.city].name}</span><span class="tag">${PTYPES[l.propertyType]}</span><span class="tag">${l.totalArea} m²</span></span></span>
      </div>`).join(""):`<p class="empty">Asnjë pronë nuk përputhet me filtrat.</p>`;
    $$(".card",listEl).forEach(c=>c.addEventListener("click",()=>location.hash="#/property/"+c.dataset.id));
  };
  $$("#ppDealSeg button",v).forEach(b=>b.addEventListener("click",()=>{
    $$("#ppDealSeg button",v).forEach(x=>x.classList.toggle("on",x===b));
    pFilters.deal=b.dataset.v; drawListings();
  }));
  ["ppType","ppBeds","ppSort","ppPriceMin","ppPriceMax","ppAreaMin","ppAreaMax"].forEach(id=>{
    const el=$("#"+id,v); if(el) el.addEventListener("input",drawListings);
  });
  if($("#ppCity",v)) $("#ppCity").addEventListener("input",drawListings);
  $("#ppClear").addEventListener("click",()=>{
    $("#ppType").value=""; if($("#ppCity",v))$("#ppCity").value=""; $("#ppBeds").value="0"; $("#ppSort").value="new";
    $("#ppPriceMin").value=""; $("#ppPriceMax").value=""; $("#ppAreaMin").value=""; $("#ppAreaMax").value="";
    drawListings();
  });
  drawListings();
  if(p.listings.length){
    await loadGeoData();
    const map=activeMap=createMap("partnerMap",online,{center:[p.listings[0].lng,p.listings[0].lat],zoom:12,nav:true});
    if(p.listings.length>1){
      const bounds=new maplibregl.LngLatBounds();
      p.listings.forEach(l=>bounds.extend([l.lng,l.lat]));
      map.fitBounds(bounds,{padding:50,maxZoom:13,duration:0});
    }
    p.listings.forEach(l=>{
      const el=document.createElement("div"); el.className="draft-pin"; el.style.cursor="pointer";
      new maplibregl.Marker({element:el,anchor:"bottom"}).setLngLat([l.lng,l.lat]).addTo(map);
      el.addEventListener("click",()=>location.hash="#/property/"+l.id);
    });
    map.highlightListingBuildings(p.listings);
  }
}
function viewStatic(kind){
  ...
  if(kind==="about"){
    v.innerHTML=`<div class="static-page"><div class="inner">
      <h1>Rreth Nesh</h1>
      <p><b>Prona</b> është platforma digjitale e pronave për të gjithë Shqipërinë.</p>
      <p>Misioni ynë është i thjeshtë: të bëjmë blerjen, shitjen dhe qiradhënien e pronave, transparente dhe të lehtë. Çdo pronë vendoset në hartë interaktive 3D, me vendndodhje të saktë dhe pamje satelitore. Kështu, prona juaj bëhet më e lehtë për t'u gjetur dhe merr ekspozim maksimal.</p>
      <h2>Çfarë ofrojmë</h2>
      <ul>
        ...
        <li><b>Publikim falas</b> — çdo pronar, agjent apo agjenci mund të publikojë pronat pa pagesë.</li>
        <li><b>Kontakt i drejtpërdrejtë</b> — blerësit dhe qiramarrësit kontaktojnë direkt me pronarin ose agjentin, nëpërmjet numrit të telefonit ose WhatsApp-it, pa ndërmjetës.</li>
        <li><b>Kërkim i avancuar dhe kërkime të ruajtura</b> — filtroni sipas çmimit, sipërfaqes, dhomave dhe veçorive, ruani kërkimin dhe merrni njoftim kur shtohen prona që përputhen me kërkesat tuaja.</li>
        <li><b>Favoritet</b> — ruani pronat që ju pëlqejnë me një klikim dhe rikthehuni për ti rishikuar kur të dëshironi.</li>
        <li><b>Promovim me ofertë ditore</b> — kush dëshiron më shumë dukshmëri, promovoni pronën tuaj nëpërmjet një oferte ditore në mënyrë që të maksimizoni shikueshmërinë dhe të rendisni më lartë në renditje pronën tuaj.</li>
        <li><b>Pagesa fleksibël</b> — rimbushni bilancin me PayPal ose me anë të kriptomonedhave (BTC, ETH, USDT, SOL).</li>
      </ul>
      <h2>Për agjentët dhe agjencitë</h2>
      <p>Krijoni llogarinë tuaj si agjent ose agjenci dhe menaxhoni të gjitha pronat tuaja në një vend të vetëm. Ndiqni performancën e çdo prone me statistika të detajuara mbi shikimet, interesimin dhe telefonatat, ndërsa promovoni pronat tuaja në mënyrë fleksibël, sipas buxhetit dhe nevojave tuaja.</p>
      <h2>Besueshmëri dhe siguri</h2>
      <p>Siguria dhe transparenca janë prioriteti ynë. Llogaritë verifikohen përmes email-it, ndërsa të dhënat dhe fjalëkalimet mbrohen me masa të sigurta enkriptimi. Pagesat për shërbimet e promovimit kryhen përmes ofruesve të besueshëm dhe të sigurt, si PayPal dhe Coinbase Commerce.</p>
      <p>Prona është një platformë ndërmjetësimi dhe nuk garanton vlefshmërinë e pronave të listuara. Përpara çdo pagese ose transaksioni, ju rekomandojmë të verifikoni pronën, pronësinë dhe dokumentacionin përkatës.</p>
    </div></div>`;
    attachFooter($(".static-page .inner",v));
    return;
  }
...
function render(){
  ...
  else if(h.startsWith("#/partner/")) viewPartnerProfile(decodeURIComponent(h.slice(10)));
  else if(h.startsWith("#/partners")) viewPartners();
