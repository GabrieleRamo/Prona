...
function getPartners(){
  ...
  listings.forEach(l=>{
    ...
    if(!groups[key]) groups[key]={key,name:l.contactName,phone:l.phone,whatsapp:false,accountType:l.accountType,cities:new Set(),count:0,listings:[]};
    const g=groups[key];
  ...
  const draw=q=>{
    ...
    grid.innerHTML=list.map(p=>{
      ...
      return `<div class="partner-card">
        <span class="avatar partner-avatar">${esc(initials(p.name))}</span>
...
async function viewPartnerProfile(key){
  ...
  v.innerHTML=`<div class="static-page partner-profile-page"><div class="inner" style="max-width:960px">
    ...
    <div class="pp-header">
      <span class="avatar pp-avatar">${esc(initials(p.name))}</span>
