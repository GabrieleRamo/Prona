...
function getPartners(){
  ...
  listings.forEach(l=>{
    ...
    if(!groups[key]) groups[key]={key,name:l.contactName,phone:l.phone,whatsapp:false,accountType:l.accountType,cities:new Set(),count:0,listings:[],avatarPhoto:l.avatarPhoto};
    const g=groups[key];
...
function viewPartners(){
  ...
  v.innerHTML=`<div class="static-page partners-page"><div class="inner" style="max-width:960px">
  ...
  const draw=q=>{
    ...
    const list=partners.filter(p=>!qq||p.name.toLowerCase().includes(qq)||[...p.cities].some(c=>c.toLowerCase().includes(qq)));
    grid.innerHTML=list.map(p=>{
      ...
      return `<div class="partner-card">
        ...
        <span class="partner-stat">${p.count} ${p.count===1?"pronë aktive":"prona aktive"}</span>
  ...
  draw("");
  $("#pSearch").addEventListener("input",e=>draw(e.target.value));
}
async function viewPartnerProfile(key){
  ...
  v.innerHTML=`<div class="static-page partner-profile-page"><div class="inner" style="max-width:960px">
    ...
    <div class="pp-grid">
      <div class="pp-main">
        ...
        <h2 class="section-title">${T(`Pronat aktive (${p.count})`,`Active properties (${p.count})`)}</h2>
  ...
  attachFooter($(".static-page .inner",v));
  const listEl=$("#ppListings");
  listEl.innerHTML=p.listings.map(l=>`
    <div class="card" data-id="${l.id}" role="button" tabindex="0">
      ...
      <span class="card-info"><span class="price">${l.dealType==="sale"?`€${fmt(l.price)}`:l.dealType==="rent"?`€${fmt(l.price)}/muaj`:`€${fmt(l.price)}/natë`}</span>
      <span class="name">${esc(l.title)}</span>
