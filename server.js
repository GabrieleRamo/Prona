...
const publicUser = u => u ? { name: u.name, email: u.email, type: u.type, balance: u.balance || 0, transactions: (u.transactions || []).slice(0, 30), verified: !!u.verified, isAdmin: isAdmin(u), favorites: u.favorites || [], savedSearches: u.savedSearches || [], plan: userPlan(u), planExpiresAt: u.planExpiresAt || null, planCancelled: !!u.planCancelled } : null;
const cleanStr = (v, max = 300) => String(v ?? "").slice(0, max);
const cleanNum = (v, max = 1e9) => Math.max(0, Math.min(max, Number(v) || 0));

function sanitizeListing(input, owner, existing) {
  ...
  return {
    ...
    photos: Array.isArray(input.photos) ? input.photos.slice(0, 24).filter(p => typeof p === "string" && p.startsWith("data:image/")) : [],
...
/* ---------------- subscription plans ---------------- */
const PLANS = {
  free:    { name: "Falas",   listings: 5,        price: 0 },
  pro:     { name: "Pro",     listings: 30,       price: 29 },
  premium: { name: "Premium", listings: Infinity, price: 79 },
};
...
function runSubscriptionBilling() {
  ...
  for (const u of db.users) {
    ...
    if (!u.planCancelled && (u.balance || 0) >= price) {
      credit(u, -price, "subscription", `Rinovim plani ${PLANS[u.plan].name}`);
      u.planExpiresAt = now + 30 * DAY;
    } else {
      u.plan = "free"; delete u.planExpiresAt; delete u.planCancelled;
    }
  }
  saveDb();
}
...
async function api(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/plan") {
    ...
    if (userPlan(user) === plan) return json(res, 400, { error: "Ky është plani juaj aktual." });
    const price = PLANS[plan].price;
    if ((user.balance || 0) < price)
      return json(res, 402, { error: `Bilanci juaj (€${(user.balance || 0).toFixed(2)}) nuk mbulon planin ${PLANS[plan].name} (€${price}/muaj). Rimbushni bilancin fillimisht.` });
    credit(user, -price, "subscription", `Plani ${PLANS[plan].name} · 30 ditë`);
    user.plan = plan; user.planExpiresAt = Date.now() + 30 * DAY; user.planCancelled = false;
    saveDb();
    return json(res, 200, { user: publicUser(user) });
  }
  ...
  if (url.pathname === "/api/listings" && req.method === "POST") {
    if (!user) return json(res, 401, { error: "Hyni fillimisht." });
    const b = await readBody(req);
    const rec = sanitizeListing(b, user.email);
    ...
    if (rec.status === "published") {
      const plan = userPlan(user), limit = PLANS[plan].listings;
  ...
  if (idMatch) {
    ...
    if (req.method === "PUT") {
      const b = await readBody(req);
      const rec = sanitizeListing(b, user.email, db.listings[i]);
...
/* ---------------- static files ---------------- */
const MIME = { ".html": "text/html; charset=utf-8", ".js": "text/javascript; charset=utf-8", ".css": "text/css; charset=utf-8", ".json": "application/json", ".png": "image/png", ".jpg": "image/jpeg", ".svg": "image/svg+xml", ".ico": "image/x-icon", ".md": "text/plain; charset=utf-8" };
