...
let db = { users: [], sessions: {}, listings: [], reports: [] };
function loadDb() {
  try { db = JSON.parse(fs.readFileSync(DB_FILE, "utf8")); }
  catch (e) { /* first run */ }
  if (!Array.isArray(db.reports)) db.reports = []; // migration for db.json files created before reports existed
}
let saveTimer = null;
function saveDb() {
...
const publicUser = u => u ? { name: u.name, email: u.email, type: u.type, balance: u.balance || 0, transactions: (u.transactions || []).slice(0, 30), verified: !!u.verified, isAdmin: isAdmin(u), favorites: u.favorites || [], savedSearches: u.savedSearches || [] } : null;
...
/* ---------------- duplicate-listing detection (soft: flags for admin review, never blocks publishing) ---------------- */
...
function findPossibleDuplicates(rec) {
  const addrKey = l => (l.city + "|" + l.street.trim().toLowerCase() + "|" + l.houseNo.trim().toLowerCase());
  const recAddr = rec.street && rec.houseNo ? addrKey(rec) : null;
  return db.listings.filter(l =>
    l.id !== rec.id && l.status === "published" && l.dealType === rec.dealType && l.propertyType === rec.propertyType &&
    (
      (rec.lng && rec.lat && haversineMeters(rec.lat, rec.lng, l.lat, l.lng) < 30) ||
      (recAddr && l.street && l.houseNo && addrKey(l) === recAddr)
    )
  ).map(l => l.id);
}
...
/* ---------------- listing limits (plans/subscriptions disabled for now — everyone gets the free tier) ---------------- */
const FREE_LISTINGS_LIMIT = 30;
const FREE_PHOTOS_LIMIT = 20;
...
const stripStats = (l, viewerEmail) =>
  l.owner === viewerEmail ? l : (({ stats, statsDaily, possibleDuplicateOf, ...rest }) => rest)(l);
...
const nearbyCache = new Map(); // key "lat,lng" (3dp) -> {t, data} — avoids re-querying Overpass for repeat views
const NEARBY_TTL = 6 * 60 * 60 * 1000; // 6 hours
...
setInterval(() => { runPromotionBilling(); }, 60 * 60 * 1000);
setTimeout(() => { runPromotionBilling(); }, 5000);
...
async function api(req, res, url) {
  ...
  if (req.method === "GET" && url.pathname === "/api/nearby") {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) return json(res, 400, { error: "Koordinata të pavlefshme." });
    const key = lat.toFixed(3) + "," + lng.toFixed(3);
    const cached = nearbyCache.get(key);
    if (cached && Date.now() - cached.t < NEARBY_TTL) return json(res, 200, cached.data);
    try {
      const query = `[out:json][timeout:20];(
        node["amenity"~"^(restaurant|cafe|bar|pub|nightclub|cinema|theatre|pharmacy|school)$"](around:1200,${lat},${lng});
        node["shop"~"^(supermarket|convenience|grocery|mall|clothes|department_store|shoes|electronics)$"](around:1200,${lat},${lng});
        node["tourism"="museum"](around:1200,${lat},${lng});
        node["highway"="bus_stop"](around:600,${lat},${lng});
        way["highway"="cycleway"](around:900,${lat},${lng});
      );out center 250;`;
      const r = await fetch("https://overpass-api.de/api/interpreter", {
        method: "POST", headers: { "Content-Type": "text/plain" }, body: query,
      });
      if (!r.ok) return json(res, 502, { error: "Overpass API nuk u përgjigj." });
      ...
      const places = []; let busStops = 0, cyclewayCount = 0, amenityCount = 0;
      (raw.elements || []).forEach(e => {
        if (e.type === "way" && e.tags && e.tags.highway === "cycleway") { cyclewayCount++; return; }
        if (e.tags && e.tags.highway === "bus_stop") { busStops++; return; }
        const cat = classify(e.tags);
        const la = e.lat ?? (e.center && e.center.lat), lo = e.lon ?? (e.center && e.center.lon);
        if (la == null || lo == null) return;
        amenityCount++;
        if (cat) places.push({ lat: la, lng: lo, name: (e.tags && e.tags.name) || "", cat });
      });
      const data = {
        places,
        walkScore: Math.min(100, Math.round((amenityCount / 25) * 100)),
        transitScore: Math.min(100, busStops * 20),
        bikeScore: Math.min(100, cyclewayCount * 25),
      };
      nearbyCache.set(key, { t: Date.now(), data });
      return json(res, 200, data);
    } catch (e) {
      return json(res, 502, { error: "Nuk u lidh dot me OpenStreetMap." });
    }
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/report") {
    const b = await readBody(req);
    const listingId = cleanStr(b.listingId, 60);
    const REASONS = ["fake", "sold", "wrong_info", "spam", "other"];
    const reason = REASONS.includes(b.reason) ? b.reason : "other";
    const note = cleanStr(b.note, 500);
    if (!listingId) return json(res, 400, { error: "Shpallje e pavlefshme." });
    const listing = db.listings.find(x => x.id === listingId);
    if (!listing) return json(res, 404, { error: "Shpallja nuk u gjet." });
    const reporterKey = user ? user.email : (req.socket.remoteAddress || "anon");
    const dup = db.reports.find(r => r.listingId === listingId && r.reporterKey === reporterKey && r.status === "open");
    if (dup) return json(res, 200, { ok: true, alreadyReported: true });
    db.reports.push({
      id: "rep-" + crypto.randomBytes(8).toString("hex"),
      listingId, reason, note,
      reporterKey, reporterEmail: user ? user.email : null,
  ...
  if (url.pathname.startsWith("/api/admin/")) {
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/overview") {
      ...
      return json(res, 200, {
        ...
        reportsOpenCount: db.reports.filter(r => r.status === "open").length,
        duplicatesOpenCount: db.listings.filter(l => l.status === "published" && (l.possibleDuplicateOf || []).length).length,
        revenue: Math.round(revenue * 100) / 100,
      });
    }
    if (req.method === "GET" && url.pathname === "/api/admin/reports") {
      const REASON_LABELS = { fake: "E rreme", sold: "E shitur/dhënë", wrong_info: "Info e gabuar", spam: "Spam", other: "Tjetër" };
      const reports = db.reports.filter(r => r.status === "open").sort((a, b) => b.createdAt - a.createdAt).map(r => {
        const l = db.listings.find(x => x.id === r.listingId);
        return { ...r, reasonLabel: REASON_LABELS[r.reason] || r.reason, listingTitle: l ? l.title : "(u fshi)", listingStatus: l ? l.status : null };
      });
      return json(res, 200, { reports });
    }
    const rm = /^\/api\/admin\/reports\/([\w-]+)\/resolve$/.exec(url.pathname);
    if (rm && req.method === "POST") {
      const r = db.reports.find(x => x.id === rm[1]);
      if (!r) return json(res, 404, { error: "Raportimi nuk u gjet." });
      r.status = "resolved"; saveDb();
      return json(res, 200, { ok: true });
    }
    ...
    if (am && req.method === "DELETE") {
      ...
      db.reports.forEach(r => { if (r.listingId === am[1] && r.status === "open") r.status = "resolved"; });
      saveDb();
      return json(res, 200, { ok: true });
    }
  ...
  if (url.pathname === "/api/listings" && req.method === "POST") {
    if (!user) return json(res, 401, { error: "Hyni fillimisht." });
    const b = await readBody(req);
    const rec = sanitizeListing(b, user.email, null, FREE_PHOTOS_LIMIT);
    ...
    if (rec.status === "published") {
      if (publishedCount(user.email, rec.id) >= FREE_LISTINGS_LIMIT)
        return json(res, 403, { error: `Mund të keni deri në ${FREE_LISTINGS_LIMIT} shpallje aktive.` });
    }
    rec.photos = storePhotos(rec.photos);
    const payErr = startPromotion(rec, null);
    if (payErr) return json(res, 402, { error: payErr });
    const dupIds = rec.status === "published" ? findPossibleDuplicates(rec) : [];
    if (dupIds.length) rec.possibleDuplicateOf = dupIds;
    db.listings.push(rec); saveDb();
    notifySavedSearches(rec); // fire-and-forget email alerts
    return json(res, 200, { listing: rec, user: publicUser(user), duplicateWarning: dupIds.length > 0 });
  }
  ...
  if (idMatch) {
    ...
    if (req.method === "PUT") {
      const b = await readBody(req);
      const rec = sanitizeListing(b, user.email, db.listings[i], FREE_PHOTOS_LIMIT);
      ...
      if (payErr) return json(res, 402, { error: payErr });
      const dupIds = rec.status === "published" ? findPossibleDuplicates(rec) : [];
      if (dupIds.length) rec.possibleDuplicateOf = dupIds; else delete rec.possibleDuplicateOf;
